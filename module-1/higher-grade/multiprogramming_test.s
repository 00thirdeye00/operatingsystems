...
PRESS_MMIO_KEY_S:	.asciiz "Type a string\n"


GIVE_STR: .asciiz "Choose size of the string..\n"
USR_SIZE: .asciiz " 1. Twenty Bytes\n 2. Fourty Bytes\n 3. Sixty Bytes\n"
...
#gets strings to print for different sizes
TWTY: .asciiz "Twenty\n" 
FRTY: .asciiz "Fourty\n"
SXTY: .asciiz "Sixty\n"

str_array: .word TWTY, FRTY, SXTY
size_array: .word 20, 40, 60

str_size: .word buf_twty, buf_frty, buf_sxty
...
__b_twty:
    ...
    li $v0,	8 #take in input
    la $a0, 0($t0) #load byte space into address
...
__b_frty:
    ...
    li $v0,	8 #take in input
    la $a0, 4($t0) #load byte space into address
    li $a1, 41 # allot the byte space for string
    move $t0,$a0 #save string to t0
    syscall
...
__b_sxty:
    ...
    li $v0,	8 #take in input
    la $a0, 8($t0) #load byte space into address
    li $a1, 61 # allot the byte space for string
    move $t0,$a0 #save string to t0
    syscall
 ...
 __normal:	
#---------------------------------get strings of different buffer sizes-----
...
job_getc_infinite_loop:
	...
	#lw $k1, 0xffff0004
	#move buffer, $k1
 	#lw  $t0, RECEIVER_DATA	# Address of receiver data register.
  	#lw  $t1, 0($t0)			# Value of receiver data register.

    #la $a0, 0($t1) #load byte space into address

    #la $a0, RECEIVER_DATA #load byte space into address
    #li $a1, 20 # allot the byte space for string
    #move $t0,$a0 #save string to t0
    #li $v0, 4
    #syscall
...
job_gets_infinite_loop:
	...
	li $v0, 4               # System call code (4) print string. 	
	la $a0, PRESS_MMIO_KEY_S  # String to print.
	syscall 		# Execute the Mars built-in system call (4) to print string.	
